    ...
    const mediaContent = {
      ...
      videos: [
        {
          ...
          url:'https://vkvideo.ru/clip-194976808_456239373',
          cover:'/assets/video-covers/cover-week-change.jpg',
          orientation:'vertical'
        },
        {
          title:'Зачем детям копить деньги в лагере?',
          url:'https://vkvideo.ru/clip-194976808_456239364',
          cover:'/assets/video-covers/cover-week-change.jpg',
          orientation:'vertical'
        },
        {
          title:'Ребёнок сам откажется от телефона за 3 дня?',
          url:'https://vkvideo.ru/clip-194976808_456239371',
          cover:'/assets/video-covers/cover-week-change.jpg',
          orientation:'vertical'
        },
        {
          title:'Как дети растут за смену в AiДаКемп',
          url:'https://vkvideo.ru/clip-194976808_456239372',
          cover:'/assets/video-covers/cover-week-change.jpg',
          orientation:'vertical'
        },
        {
          title:'Ещё один реальный кейс смены',
          url:'https://vkvideo.ru/clip-194976808_456239362',
          cover:'/assets/video-covers/cover-week-change.jpg',
          orientation:'vertical'
        }
      ],
      ...
      socials: [
        { key:'Telegram', label:'TG', href:'https://t.me/aidacamp', icon:'/assets/icons/telegram.svg' },
        ...
        { key:'LinkedIn', label:'IN', href:'https://www.linkedin.com/company/%D0%B8%D1%82-%D0%BB%D0%B0%D0%B3%D0%B5%D1%80%D1%8C-%D0%B4%D0%BB%D1%8F-%D0%B4%D0%B5%D1%82%D0%B5%D0%B9-%D0%B0%D0%B9%D0%B4%D0%B0%D0%BA%D0%B5%D0%BC%D0%BF/?viewAsMember=true', icon:'/assets/icons/linkedin.svg' },
        { key:'TikTok', label:'TT', href:'https://www.tiktok.com/@aidacodit0', icon:'/assets/icons/tiktok.svg' },
        { key:'Pinterest', label:'PI', href:'https://ru.pinterest.com/pbalgoritmika/', icon:'/assets/icons/pinterest.svg' },
        { key:'Yappy', label:'YA', href:'https://yappy.media/n/acceptable.lizardxw?utm_source=url&utm_medium=share&utm_campaign=profile', icon:'/assets/icons/yappy.svg' }
      ],
      reviews: [
        {
    ...
    function videoSourceLabel(url){
      const sourceName = resolveVideoSource(url).sourceName;
      return sourceName === 'источнике' ? 'Видео' : sourceName;
    }
    ...
    function getPrimaryActionState(){
      ...
      if(state.offerStage === 0){
        return {
          text:'Проверить условия и цену',
          disabled:false,
          hint:'Покажем цену, код бронирования и зафиксируем условия на устройстве.'
        };
      }
    ...
    function applyBookingStageClass(prefix){
      const card = document.getElementById(`${prefix}BookingCard`);
      if(!card) return;
    ...
    function renderGuidedState(prefix){
      ...
      if(!shiftList || !hint || !ctaWrap || !ageTabs || !ageChip || !ageChipText || !shiftChip || !shiftChipText) return;
    ...
    function renderBookingInfo(targetInfoId, targetTitleId, targetLeadId, targetBtnId, targetHintId){
      ...
      if(!shift){
        ...
        if(lead) lead.textContent = 'Сначала выберите возраст, затем смену — и мы покажем актуальную цену и условия.';
      ...
      if(title) title.textContent = state.offerPrice ? 'Ваша цена готова' : 'Смена выбрана';
    ...
    function socialIconMarkup(item){
      if(item?.icon){
        return `<img class="ac-icon" src="${item.icon}" alt="" aria-hidden="true">`;
      }
      return `<span class="social-glyph-text">${item?.label || '•'}</span>`;
    }
    ...
    function renderMediaSections(){
      ...
      if (videoList) {
        const sharedVideoCover = '/assets/video-covers/cover-week-change.jpg';
        videoList.innerHTML = mediaContent.videos.map((item, idx) => `
          <div class="video-card" data-video="${item.url}">
            <div class="video-poster" data-video-order="${idx + 1}">
              <img src="${sharedVideoCover}" alt="${item.title}">
              <div class="video-source-tag">${videoSourceLabel(item.url)}</div>
      ...
      if (mediaContent.videos[0]) {
        const mainVideo = mediaContent.videos[0];
        const isMainVertical = mainVideo.orientation === 'vertical';
        if (mainVideoBtn) {
          mainVideoBtn.href = mainVideo.url;
          mainVideoBtn.dataset.video = mainVideo.url;
        }
        if (mainVideoBox) {
          mainVideoBox.href = mainVideo.url;
          mainVideoBox.dataset.video = mainVideo.url;
          mainVideoBox.classList.toggle('main-video-vertical', isMainVertical);
          const mainCover = mainVideo.cover || '';
          if(mainCover){
            mainVideoBox.classList.add('main-video-cover');
            mainVideoBox.style.backgroundImage = `url("${mainCover}")`;
            mainVideoBox.innerHTML = `
              <div class="video-play"><span>▶</span></div>
            `;
          } else {
            mainVideoBox.classList.remove('main-video-cover');
            mainVideoBox.style.backgroundImage = '';
            mainVideoBox.innerHTML = `
              <div class="video-play"><span>▶</span></div>
            `;
          }
      ...
      if (socialsGrid) {
        socialsGrid.innerHTML = mediaContent.socials.map(item => `
          <a class="social-link" href="${item.href}" target="_blank" rel="noopener noreferrer">
            <div class="social-glyph">${socialIconMarkup(item)}</div>
            <span>${item.key}</span>
          </a>
        `).join('');
      }
      ...
      if (teamGrid) {
        ...
        const [daryaCard, nikitaCard, ...tailCards] = mediaContent.team;
        const coreCards = [daryaCard, nikitaCard]
          .filter(Boolean)
          .map(renderTeamCard)
          .join('');
        const carouselCards = (tailCards.length ? tailCards : mediaContent.team.slice(2))
          .map(renderTeamCard)
          .join('');
        ...
        teamGrid.innerHTML = `
          <div class="team-layout">
            ...
            <div class="team-carousel">${carouselCards}</div>
          </div>
        `;
      }
      ...
      if (mobileInlineSocials) {
        mobileInlineSocials.innerHTML = mediaContent.socials.map(item => `
          <a class="mobile-social-link" href="${item.href}" target="_blank" rel="noopener noreferrer" aria-label="${item.key}">
            <span class="mobile-social-icon">${socialIconMarkup(item)}</span>
          </a>
        `).join('');
      }
    }
    ...
    function scrollToSection(id){
      ...
      const mobileMap = {
        ...
        'section-legal':'mobile-section-legal'
      };
