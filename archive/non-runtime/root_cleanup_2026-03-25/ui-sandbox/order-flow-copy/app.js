(function () {
  ...
  function renderShiftList() {
    if (!state.ui.entryStarted) {
      dom.shiftList.innerHTML = '<p class="of-help">Нажмите «Выбрать смену и цену», чтобы открыть список смен.</p>';
    ...
    dom.shiftList.innerHTML = list.map(function (shift) {
      ...
      return (
        '<div class="of-shift-item">' +
          '<div class="of-shift-item__top">' +
            '<div>' +
              '<div class="of-shift-item__title">' + escapeHtml(shift.title + " • " + shift.dateLabel) + '</div>' +
              '<div class="of-mini">Возраст: ' + escapeHtml(shift.ageRange) + ' • ' + escapeHtml(shift.location) + '</div>' +
            '</div>' +
            '<span class="of-badge ' + badge.className + '">' + badge.text + '</span>' +
          '</div>' +
          '<div class="of-mini">Заполнено ' + occupancy + '% мест</div>' +
          '<button class="of-btn of-btn--line" type="button" data-action="select-shift" data-shift-id="' + shift.id + '">Открыть карточку смены</button>' +
        '</div>'
      );
    }).join("");
  }
  ...
  function renderShiftCard() {
    ...
    if (activeOffer) {
      ...
      priceBlock = (
        '<div class="of-price-box">' +
          '<div class="of-mini">' + label + '</div>' +
          '<div class="of-price-main">' + formatPrice(activeOffer.improvedPrice) + '</div>' +
          '<div class="of-mini">Действует до ' + formatLocalDateTime(activeOffer.expiresAt) + '</div>' +
        '</div>'
      );
      ...
      priceBlock = (
        '<div class="of-price-box">' +
          '<div class="of-mini">Стоимость</div>' +
          '<div class="of-price-main">' + formatPrice(shift.basePrice) + '</div>' +
        '</div>'
      );
    ...
    var expireDebug = activeOffer
      ? '<button class="of-link" type="button" data-action="sandbox-expire-offer">sandbox: завершить текущий offer</button>'
      : "";
    ...
    dom.shiftCard.innerHTML = (
      '<div class="of-card__head">' +
        ...
        '<span class="of-badge ' + badge.className + '">' + badge.text + '</span>' +
      '</div>' +
      '<p class="of-help">' + escapeHtml(shift.summary) + '</p>' +
      '<div class="of-mini">Возраст: ' + escapeHtml(shift.ageRange) + ' • Локация: ' + escapeHtml(shift.location) + '</div>' +
      '<div class="of-mini">Заполнено ' + occupancy + '% мест</div>' +
      priceBlock +
      '<button class="of-btn of-btn--primary" type="button" data-action="open-main-cta">' + ctaLabel + '</button>' +
      '<p class="of-help">' + escapeHtml(helper) + '</p>' +
      '<div class="of-card__head">' +
        '<button class="of-link" type="button" data-action="clear-hero">Очистить Hero-блоки</button>' +
        expireDebug +
      '</div>'
    );
  }
  ...
  function renderBooking() {
    ...
    dom.bookingBlock.innerHTML = (
      ...
      '<p class="of-help">' + escapeHtml(offerText) + '</p>' +
      (state.ui.bookingError ? ('<div class="of-alert">' + escapeHtml(state.ui.bookingError) + '</div>') : "") +
      '<div class="of-grid-2">' +
      ...
      '</label>' +
      '<button class="of-btn of-btn--primary" type="button" data-action="submit-booking">Подтвердить бронь</button>'
    );
  }
  ...
  function renderSuccess() {
    ...
    dom.successBlock.innerHTML = (
      ...
      '<p class="of-success">Заявка принята. Менеджер свяжется с вами для подтверждения.</p>' +
      '<p class="of-help">Смена: ' + escapeHtml(success.shiftTitle) + '</p>' +
      '<p class="of-help">Цена фиксации: ' + formatPrice(success.price) + '</p>' +
      '<p class="of-help">Тип: ' + escapeHtml(success.offerType) + '</p>'
    );
  }
  ...
  function renderHeroSlot(node, data, sideLabel) {
    if (!data) {
      node.innerHTML = '<span class="of-muted">Слот пуст</span>';
      return;
    }

    var ttlLine = data.expiresAt ? ('<div class="of-mini">до ' + formatLocalDateTime(data.expiresAt) + '</div>') : "";
    node.innerHTML = (
      '<div class="of-hero-slot__head">' +
        '<strong>' + sideLabel + ' Hero</strong>' +
        '<button class="of-link" type="button" data-action="clear-hero">Очистить</button>' +
      '</div>' +
      '<div>' + escapeHtml(data.title || "") + '</div>' +
      '<div class="of-mini">' + escapeHtml(data.dateLabel || "") + '</div>' +
      (data.price ? ('<div class="of-price-main" style="font-size:18px">' + formatPrice(data.price) + '</div>') : "") +
      ttlLine
    );
  ...
  function renderProgressModal(modal) {
    ...
    dom.modalRoot.innerHTML = (
      '<section class="of-modal" role="dialog" aria-modal="true">' +
        '<div class="of-modal__head">' +
          '<h3>' + escapeHtml(modal.type === "final_24h" ? "Пробуем найти для вас финальную цену" : "Ищем для вас лучшую цену") + '</h3>' +
          '<button class="of-btn of-btn--ghost" type="button" data-action="modal-close">Закрыть</button>' +
        '</div>' +
        '<div class="of-progress"><div class="of-progress__bar" style="width:' + progress + '%"></div></div>' +
        '<ol class="of-step-list">' +
  ...
  function renderResultModal(modal) {
    ...
    if (!canUseKnownPhone) {
      ...
      phoneSection = (
        '<div class="of-field">' +
          '<label>Телефон</label>' +
          '<input class="of-input" name="modal-phone" type="tel" inputmode="tel" value="' + escapeHtml(modal.phoneInput || "") + '" placeholder="79991234567">' +
        '</div>' +
        '<label class="of-consent">' +
          '<input type="checkbox" name="modal-consent" ' + (modal.consent ? "checked" : "") + '>' +
          '<span>Согласен(на) на обработку персональных данных</span>' +
        '</label>' +
        '<p class="of-help">' + escapeHtml(optionalText) + '</p>'
      );
    }
    ...
    dom.modalRoot.innerHTML = (
      '<section class="of-modal" role="dialog" aria-modal="true">' +
        ...
        '</div>' +
        '<div class="of-price-box">' +
          '<div class="of-price-old">' + formatPrice(offer.basePrice) + '</div>' +
          '<div class="of-price-main">' + formatPrice(offer.improvedPrice) + '</div>' +
          '<div class="of-mini">Действует до ' + formatLocalDateTime(offer.expiresAt) + '</div>' +
        '</div>' +
        '<p class="of-help">' + escapeHtml(isFinal
          ? "Это финальное улучшение цены. Зафиксируем на 24 часа."
          : "Эту цену можно зафиксировать за вами на 72 часа.") + '</p>' +
        phoneSection +
        (modal.error ? ('<div class="of-alert">' + escapeHtml(modal.error) + '</div>') : "") +
        '<div class="of-actions">' +
          '<button class="of-btn of-btn--primary" type="button" data-action="offer-confirm">' +
            (canUseKnownPhone
          ...
          '</button>' +
          '<button class="of-btn of-btn--line" type="button" data-action="offer-save-browser">Сохранить только в браузере</button>' +
        '</div>' +
      '</section>'
    );
  }
  ...
  function getStatusBadge(occupancyPercent) {
    if (occupancyPercent >= 90) {
      return { text: "Почти нет мест", className: "of-badge--danger" };
    }
    if (occupancyPercent >= 80) {
      return { text: "Мест почти нет", className: "of-badge--warn" };
    }
  ...
  function fieldHtml(label, name, value, type) {
    ...
    return (
      '<div class="of-field">' +
        ...
        '<input class="of-input" name="' + name + '" type="' + safeType + '" value="' + escapeHtml(value || "") + '">' +
      '</div>'
    );
  }
