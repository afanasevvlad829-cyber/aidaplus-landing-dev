# Report — Refactor Cleanup (текущий этап)

## Задача
Рефакторинг и расчистка фронтенд-кода без целевого изменения пользовательского поведения.

## Статус
Частично выполнено (безопасный cleanup-pass с минимальным diff).

## Что реально сделано
- `index.html`:
  - удалён временный inline-блок `<style id="ac-manual-hotfix-style">`;
  - удалён временный inline-блок `<script id="ac-manual-hotfix-script">`;
  - убрана дублирующая manual-логика для nav/contact, которая конфликтовала с основной логикой в `src/scripts/main.js`.
- `src/scripts/main.js`:
  - добавлены базовые общие helper-утилиты: `chunkArray(...)`, `debounce(...)`;
  - dev-switches собраны в единый блок `AC_DEV_CONFIG` с сохранением текущих флагов (`AC_DEV_REMOTE_TEAM_REFRESH_ENABLED`, `AC_DEV_RUTUBE_VIDEO_FEED_ENABLED`);
  - убрано дублирование локальных `chunk(...)` и ручных `resizeTimer` в каруселях/слайдерах (reviews/team/full photos/full video/full reviews/full team);
  - добавлены крупные секционные заголовки для повышения читаемости и навигации по файлу.
- `src/styles/main.css`:
  - добавлены секционные комментарии по слоям (nav/background/hero/components);
  - удалён один лишний `!important` в `.ac-hero-contact-card.is-open` без изменения поведения.

## Что намеренно не трогалось
- business logic бронирования;
- состав и источники данных (смены/отзывы/команда/видео/фото/FAQ);
- общий UX и layout секций (кроме удаления явно временных hotfix-дублей).

## Ограничения текущего прохода
- Полного распила `main.js` на физические модули не делалось (чтобы не повышать риск регрессий).
- Глубокая декомпозиция CSS на отдельные файлы не делалась; выполнена структурная расчистка внутри текущего `main.css`.
