...
import { isFinalFunnelStep } from "./features/funnel.js";
...
var state = createInitialState(initialShift);
  ...
  function getCompactLeftPreview() {
    if (state.mode !== "compact" || state.activeTab === "info") return null;

    if (state.activeTab === "aiprogram") {
      return {
        title: CONTENT_MAP.sectionTitles.ai,
        subtitle: CONTENT_MAP.aiCopy[0] || "",
        progress: CONTENT_MAP.aiStats[0] ? CONTENT_MAP.aiStats[0].value + " " + CONTENT_MAP.aiStats[0].label : "",
        benefits: [
          { icon: ICON_MAP.sparkle, text: CONTENT_MAP.aiStats[1] ? CONTENT_MAP.aiStats[1].label : "AI-навыки" },
          { icon: ICON_MAP.money, text: CONTENT_MAP.aiStats[2] ? CONTENT_MAP.aiStats[2].label : "Рост рынка" },
          { icon: ICON_MAP.robot, text: CONTENT_MAP.aiStats[3] ? CONTENT_MAP.aiStats[3].label : "Практика" }
        ]
      };
    }
    ...
    if (state.activeTab === "location") {
      return {
        title: CONTENT_MAP.sectionTitles.location,
        subtitle: CONTENT_MAP.location.where[0] || "",
        progress: CONTENT_MAP.ui.locationWhereTitle,
        benefits: [
          { icon: ICON_MAP.pool, text: CONTENT_MAP.location.nearby[0] || "" },
          { icon: ICON_MAP.check, text: CONTENT_MAP.location.nearby[1] || "" },
          { icon: ICON_MAP.lock, text: CONTENT_MAP.location.nearby[2] || "" }
        ]
      };
    }
    ...
    if (state.activeTab === "photo") {
      return {
        title: CONTENT_MAP.sectionTitles.photos,
        subtitle: "Фото и атмосфера лагеря",
        progress: "Открывайте фото по клику",
        benefits: [
          { icon: ICON_MAP.search, text: "Категории: " + CONTENT_MAP.photoCategories.map(function (c) { return c.label; }).join(", ") },
          { icon: ICON_MAP.chevronLeft, text: "Листайте стрелками влево/вправо" },
          { icon: ICON_MAP.chevronRight, text: "Фото открываются в popup" }
        ]
      };
    }
    ...
    if (state.activeTab === "video") {
      return {
        title: CONTENT_MAP.sectionTitles.video,
        subtitle: CONTENT_MAP.videos[0] ? CONTENT_MAP.videos[0].title : "",
        progress: "Видео со смен",
        benefits: [
          { icon: ICON_MAP.play, text: CONTENT_MAP.videos[0] ? CONTENT_MAP.videos[0].title : "" },
          { icon: ICON_MAP.play, text: CONTENT_MAP.videos[1] ? CONTENT_MAP.videos[1].title : "" },
          { icon: ICON_MAP.play, text: CONTENT_MAP.videos[2] ? CONTENT_MAP.videos[2].title : "" }
        ]
      };
    }
    ...
    if (state.activeTab === "faq") {
      return {
        title: CONTENT_MAP.sectionTitles.faq,
        subtitle: "Частые вопросы родителей",
        progress: CONTENT_MAP.faqTabs[0] ? CONTENT_MAP.faqTabs[0].label : "",
        benefits: [
          { icon: ICON_MAP.med, text: FAQ_DATA[0] && FAQ_DATA[0].items[0] ? FAQ_DATA[0].items[0].q : "" },
          { icon: ICON_MAP.lock, text: FAQ_DATA[1] && FAQ_DATA[1].items[0] ? FAQ_DATA[1].items[0].q : "" },
          { icon: ICON_MAP.food, text: FAQ_DATA[2] && FAQ_DATA[2].items[0] ? FAQ_DATA[2].items[0].q : "" }
        ]
      };
    }
    ...
    if (state.activeTab === "reviews") {
      return {
        title: CONTENT_MAP.sectionTitles.reviews,
        subtitle: CONTENT_MAP.reviews[0] ? CONTENT_MAP.reviews[0].quote : "",
        progress: CONTENT_MAP.reviews[0] ? CONTENT_MAP.reviews[0].name : "",
        benefits: [
          { icon: ICON_MAP.fire, text: CONTENT_MAP.reviews[0] ? CONTENT_MAP.reviews[0].meta : "" },
          { icon: ICON_MAP.fire, text: CONTENT_MAP.reviews[1] ? CONTENT_MAP.reviews[1].name : "" },
          { icon: ICON_MAP.fire, text: CONTENT_MAP.reviews[2] ? CONTENT_MAP.reviews[2].name : "" }
        ]
      };
    }
    ...
    if (state.activeTab === "team") {
      return {
        title: CONTENT_MAP.sectionTitles.team,
        subtitle: CONTENT_MAP.team[0] ? CONTENT_MAP.team[0].name + " — " + CONTENT_MAP.team[0].role : "",
        progress: "Команда лагеря",
        benefits: [
          { icon: ICON_MAP.check, text: CONTENT_MAP.team[0] ? CONTENT_MAP.team[0].name : "" },
          { icon: ICON_MAP.check, text: CONTENT_MAP.team[1] ? CONTENT_MAP.team[1].name : "" },
          { icon: ICON_MAP.check, text: CONTENT_MAP.team[2] ? CONTENT_MAP.team[2].name : "" }
        ]
      };
    }
  ...
  function setReviewPage(nextPage) {
    var maxPage = Math.max(0, Math.ceil(CONTENT_MAP.reviews.length / getReviewItemsPerPage()) - 1);
    var safePage = clamp(nextPage, 0, maxPage);
  ...
  function renderInfoCard() {
    var profile = findProfileByAge(state.age);
    var compactPreview = getCompactLeftPreview();
    var heroBenefits = compactPreview && compactPreview.benefits && compactPreview.benefits.length
      ? compactPreview.benefits
      : getHeroBenefits();
    ...
    if (title) title.textContent = compactPreview ? compactPreview.title : profile.title;
    if (subtitle) subtitle.textContent = compactPreview ? compactPreview.subtitle : HERO_SUBTITLE_STATIC;
    if (progress) progress.textContent = compactPreview ? compactPreview.progress : profile.progress;
    if (ageText) {
  ...
  function renderReviewsSectionMarkup() {
    ...
    var items = CONTENT_MAP.reviews.slice(start, start + reviewItemsPerPage);
    var maxPage = Math.max(0, Math.ceil(CONTENT_MAP.reviews.length / reviewItemsPerPage) - 1);
    var cards = "";
  ...
  function renderContactOverlay() {
    return (
      ...
      '<a class="ac-contact-item" href="tel:+74951234567"><img class="ac-icon ac-icon--sm ac-contact-item__icon" src="' + ICON_MAP.phoneCity + '" alt="" aria-hidden="true"><span><strong>Городской телефон</strong><small>+7 (495) 123-45-67</small></span></a>' +
      '<a class="ac-contact-item" href="tel:+79688086455"><img class="ac-icon ac-icon--sm ac-contact-item__icon" src="' + ICON_MAP.phoneMobile + '" alt="" aria-hidden="true"><span><strong>Мобильный телефон</strong><small>+7 (968) 808-64-55</small></span></a>' +
      '<a class="ac-contact-item" href="https://t.me/aidacodit" target="_blank" rel="noopener"><img class="ac-icon ac-icon--sm ac-contact-item__icon" src="' + ICON_MAP.telegram + '" alt="" aria-hidden="true"><span><strong>Telegram</strong><small>@aidacodit</small></span></a>' +
      '<a class="ac-contact-item" href="https://wa.me/79688086455" target="_blank" rel="noopener"><img class="ac-icon ac-icon--sm ac-contact-item__icon" src="' + ICON_MAP.whatsapp + '" alt="" aria-hidden="true"><span><strong>WhatsApp</strong><small>Написать в WhatsApp</small></span></a>' +
      "</div>" +
      "</details>" +
  ...
  function handleClick(event) {
    ...
    var contactCandidate = event.target.closest('[data-action="open-contact"]');
    var allowDuringAgeGate = !!tabCandidate || !!contactCandidate;
    ...
    if (isAgeGateLocked()) {
      ...
      if (!inAgeInput && !inAuditPanelsForGate && !allowDuringAgeGate) {
        event.preventDefault();
        event.stopPropagation();
        nudgeAgeSelection();
        return;
    ...
    if (nextStep) {
      if (isFinalFunnelStep(state.step, SHIFTS.length)) {
